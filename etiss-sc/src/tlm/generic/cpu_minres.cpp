...
etiss_int32 system_call_iread(void *handle, ETISS_CPU *cpu, etiss_uint64 addr,
                              etiss_uint32 length) {
  return static_cast<etiss_sc::CPU *>(handle)->systemCallIRead(cpu, addr,
                                                               length);
}

etiss_int32 system_call_iwrite(void *handle, ETISS_CPU *cpu, etiss_uint64 addr,
                               etiss_uint8 *buffer, etiss_uint32 length) {
  return static_cast<etiss_sc::CPU *>(handle)->systemCallIWrite(cpu, addr,
                                                                buffer, length);
}

etiss_int32 system_call_dread(void *handle, ETISS_CPU *cpu, etiss_uint64 addr,
                              etiss_uint8 *buffer, etiss_uint32 length) {
  return static_cast<etiss_sc::CPU *>(handle)->systemCallDRead(cpu, addr,
                                                               buffer, length);
}

etiss_int32 system_call_dwrite(void *handle, ETISS_CPU *cpu, etiss_uint64 addr,
                               etiss_uint8 *buffer, etiss_uint32 length) {
  return static_cast<etiss_sc::CPU *>(handle)->systemCallDWrite(cpu, addr,
                                                                buffer, length);
}

etiss_int32 system_call_dbg_read(void *handle, etiss_uint64 addr,
                                 etiss_uint8 *buffer, etiss_uint32 length) {
  return static_cast<etiss_sc::CPU *>(handle)->systemCallDbgRead(addr, buffer,
                                                                 length);
}

etiss_int32 system_call_dbg_write(void *handle, etiss_uint64 addr,
                                  etiss_uint8 *buffer, etiss_uint32 length) {
  return static_cast<etiss_sc::CPU *>(handle)->systemCallDbgWrite(addr, buffer,
                                                                  length);
}

etiss_sc::CPU::IRQ::IRQ(sc_core::sc_module_name name, size_t id,
                        etiss::InterruptHandler *irq_handler)
    : sc_module(name), id_{id}, irq_handler_{irq_handler} {
  if (!irq_handler_) {
    SC_REPORT_FATAL(ID_ETISS_SC_CPU,
                    "invalid irq_handler passed in CPU::IRQ::IRQ()");
  }
...
void etiss_sc::CPU::IRQ::execute() {
  if (irq_i_.read()) {
    irq_handler_->setLine(id_, true,
                          static_cast<etiss::uint64>(
                              sc_core::sc_time_stamp().to_seconds() * 1e12));
  } else {
    irq_handler_->setLine(id_, false,
                          static_cast<etiss::uint64>(
                              sc_core::sc_time_stamp().to_seconds() * 1e12));
  }
}
...
void etiss_sc::CPU::ResetTerminatePlugin::reset(bool value) {
  switch (state_) {
    ...
    default:
      SC_REPORT_FATAL(ID_ETISS_SC_CPU,
                      "undefined state in CPU::ResetTerminatePlugin::reset()");
  }
}

void etiss_sc::CPU::ResetTerminatePlugin::terminate() { terminate_ = true; }
...
etiss_sc::CPU::CPU(sc_core::sc_module_name name, CPUParams &&cpu_params)
  ...
  data_sock_i_ =
      std::make_unique<tlm_utils::simple_initiator_socket<CPU>>("data_socket");
  //    instr_sock_i_ =
  //    std::make_unique<tlm_utils::simple_initiator_socket<CPU>>("instr_socket");
...
etiss_sc::CPU::~CPU() {
  auto cpu_time = sc_core::sc_time{
      static_cast<double>(etiss_core_->getState()->cpuTime_ps), sc_core::SC_PS};
  if (status_ == CPUStatus::ACTIVE) {
    auto clk_period_ns =
        etiss::cfg().get<int>("arch.cpu_cycle_time_ps", 10000) * 1000;
    if (cpu_time + sc_core::sc_time{100.0 * clk_period_ns, sc_core::SC_NS} >
...
void etiss_sc::CPU::setup() {
  etiss_core_ =
      etiss::CPUCore::create(etiss::cfg().get<std::string>("arch.cpu", ""),
                             "core" + std::to_string(CPU::id));

  if (!etiss_core_) {
    XREPORT_FATAL("failed to create ETISS-CPUCore in CPU::setup()");
  }
  etiss::VirtualStruct::root()->mountStruct("core" + std::to_string(CPU::id),
                                            etiss_core_->getStruct());
  ...
  if (etiss_core_->getInterruptVector()) {
    irq_handler_ = std::make_shared<etiss::InterruptHandler>(
        etiss_core_->getInterruptVector(), etiss_core_->getArch(),
        cpu_params_.irq_handler_type_, false);
    etiss_core_->addPlugin(irq_handler_);

    auto num_irq = std::min(
        cpu_params_.num_irqs_,
        static_cast<size_t>(etiss_core_->getInterruptVector()->width() * 8));
    for (size_t i = 0; i < num_irq; ++i) {
      irq_i_.emplace_back(std::make_unique<IRQ>(
          std::string{"irq_i_" + std::to_string(i)}.c_str(), i,
          irq_handler_.get()));
  ...
  if (etiss::cfg().get<bool>("etiss.log_pc", false)) {
    addPcTraceLogger(
        etiss_core_,
        etiss::cfg().get<std::string>("etiss.output_path_prefix", "") +
            "pc_trace.bin",
...
void etiss_sc::CPU::setupDMI(uint64_t addr) {
  ...
  if (!((*data_sock_i_)->get_direct_mem_ptr(payload_, dmi_objects_.front()) &&
        dmi_objects_.front().get_dmi_ptr())) {
    dmi_objects_.pop_front();
    XREPORT_FATAL("DMI not successful in CPU::setupDMI()");
  }
}

size_t etiss_sc::CPU::getNumIRQs() const { return cpu_params_.num_irqs_; }
...
etiss_int32 etiss_sc::CPU::systemCallIRead(ETISS_CPU *cpu, etiss_uint64 addr,
                                           etiss_uint32 length) {
  ...
  //    transaction(cpu, addr, buffer.data(), length, tlm::TLM_READ_COMMAND,
  //    *instr_sock_i_);
  transaction(cpu, addr, buffer.data(), length, tlm::TLM_READ_COMMAND,
              *data_sock_i_);
...
etiss_int32 etiss_sc::CPU::systemCallIWrite(ETISS_CPU *cpu, etiss_uint64 addr,
                                            etiss_uint8 *buffer,
                                            etiss_uint32 length) {
  auto return_val = reset_terminate_handler_->execute();
  if (return_val != etiss::RETURNCODE::NOERROR) {
  ...
  //    transaction(cpu, addr, buffer, length, tlm::TLM_WRITE_COMMAND,
  //    *instr_sock_i_);
...
etiss_int32 etiss_sc::CPU::systemCallDRead(ETISS_CPU *cpu, etiss_uint64 addr,
                                           etiss_uint8 *buffer,
                                           etiss_uint32 length) {
  auto return_val = reset_terminate_handler_->execute();
  if (return_val != etiss::RETURNCODE::NOERROR) {
...
etiss_int32 etiss_sc::CPU::systemCallDWrite(ETISS_CPU *cpu, etiss_uint64 addr,
                                            etiss_uint8 *buffer,
                                            etiss_uint32 length) {
  auto return_val = reset_terminate_handler_->execute();
  if (return_val != etiss::RETURNCODE::NOERROR) {
...
etiss_int32 etiss_sc::CPU::systemCallDbgWrite(etiss_uint64 addr,
                                              etiss_uint8 *buffer,
                                              etiss_uint32 length) {
  auto response_len = dbgTransaction(addr, buffer, length,
                                     tlm::TLM_READ_COMMAND, *data_sock_i_);
...
void etiss_sc::CPU::transaction(
  ...
  static auto sim_time = sc_core::sc_time{
      static_cast<double>(etiss::cfg().get<int>("vp.simulation_time_us", 0)),
      sc_core::SC_US};
  if (sim_time != sc_core::SC_ZERO_TIME &&
      sc_core::sc_time_stamp() >= sim_time) {
    etiss::log(
        etiss::WARNING,
        "Skipped memory access because transaction overran simulation time");
  ...
  for (auto d : dmi_objects_) {
    if (addr >= d.get_start_address() && addr <= d.get_end_address()) {
      if (cmd == tlm::TLM_WRITE_COMMAND && d.is_write_allowed()) {
        dmiAccess(d.get_dmi_ptr() + addr - d.get_start_address(), buffer,
                  length);
        payload_.set_response_status(tlm::TLM_OK_RESPONSE);
        time_offset += d.get_write_latency() * length;
      } else if (cmd == tlm::TLM_READ_COMMAND && d.is_read_allowed()) {
        dmiAccess(buffer, d.get_dmi_ptr() + addr - d.get_start_address(),
                  length);
        payload_.set_response_status(tlm::TLM_OK_RESPONSE);
        time_offset += d.get_write_latency() * length;
      } else {
...
sc_core::sc_time etiss_sc::CPU::getTimeOffset(ETISS_CPU *cpu) {
  return (sc_core::sc_time(cpu->cpuTime_ps, sc_core::SC_PS) -
          sc_core::sc_time_stamp());
}

void etiss_sc::CPU::updateCPUTime(ETISS_CPU *cpu,
                                  const sc_core::sc_time &time_offset) {
  cpu->cpuTime_ps =
      (sc_core::sc_time_stamp().to_seconds() + time_offset.to_seconds()) * 1e12;
}

void etiss_sc::CPU::updateSystemCTime(sc_core::sc_time &time_offset) {
  if (sc_core::sc_time_stamp() == sc_core::SC_ZERO_TIME ||
      time_offset >
          sc_core::sc_time{static_cast<double>(quantum_), sc_core::SC_PS}) {
    wait(time_offset);
    time_offset = sc_core::sc_time{0, sc_core::SC_PS};
  }
}

void etiss_sc::CPU::configurePayload(uint64_t addr, tlm::tlm_command cmd,
...
etiss_sc::CPUFactory::CPUFactory(const etiss_sc::Config &cfg,
